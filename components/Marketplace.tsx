...
import { getMarketplaceProjects } from '../services/firebaseService';
...
const ProjectCard: React.FC<{project: MarketplaceProject, onViewProject: (p: MarketplaceProject) => void, onViewProfile: (id: string) => void}> = ({ project, onViewProject, onViewProfile }) => (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg flex flex-col justify-between border border-gray-200 dark:border-gray-700 hover:border-blue-500 transition-all duration-300 h-full transform hover:-translate-y-1 group overflow-hidden">
        <div className="p-5 flex flex-col flex-grow">
            <div>
                ...
                <button onClick={(e) => { e.stopPropagation(); onViewProfile(project.creatorId); }} className="text-xs text-gray-500 dark:text-gray-500 mb-3 hover:text-blue-500 dark:hover:text-blue-400 hover:underline flex items-center gap-1">
...
const ProjectCarousel: React.FC<{
    ...
    onViewProject: (project: MarketplaceProject) => void;
    onViewProfile: (userId: string) => void;
}> = ({ title, projects, onViewProject, onViewProfile }) => {
            ...
            <div ref={scrollContainerRef} className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide" style={{ scrollSnapType: 'x mandatory' }}>
                {projects.map(project => (
                     <div key={project.id} className="flex-shrink-0 w-80" style={{ scrollSnapAlign: 'start' }}>
                        <ProjectCard project={project} onViewProject={onViewProject} onViewProfile={onViewProfile} />
                    </div>
                ))}
            </div>
...
const Marketplace: React.FC<MarketplaceProps> = ({ onViewProject, onExit, onViewProfile }) => {
    ...
    const [activeTab, setActiveTab] = React.useState<'discover' | 'browse'>('discover');
            ...
            <main className="flex-1 p-4 md:p-8">
                {isLoading ? (
                    ...
                    <>
                        {activeTab === 'discover' && (
                             <div>
                                <ProjectCarousel title="Featured Projects" projects={featuredProjects} onViewProject={onViewProject} onViewProfile={onViewProfile} />
                                <ProjectCarousel title="Highest Rated" projects={highestRated} onViewProject={onViewProject} onViewProfile={onViewProfile} />
                                <ProjectCarousel title="Most Downloaded" projects={mostDownloaded} onViewProject={onViewProject} onViewProfile={onViewProfile} />
                                <ProjectCarousel title="Most Recent" projects={mostRecent} onViewProject={onViewProject} onViewProfile={onViewProfile} />
                            </div>
                        )}
                        ...
                        {activeTab === 'browse' && (
                             <div className="border-t border-gray-200 dark:border-gray-700 pt-8">
                                ...
                                {filteredProjects.length === 0 ? (
                                    <div className="text-center text-gray-500 bg-white dark:bg-gray-800 p-8 rounded-md border border-gray-200 dark:border-gray-700">No projects match your search.</div>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                        {filteredProjects.map(project => (
                                            <ProjectCard key={project.id} project={project} onViewProject={onViewProject} onViewProfile={onViewProfile}/>
                                        ))}
                                    </div>
                                )}
                            </div>
